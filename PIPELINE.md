...
10. **UX 要有交互与动效(2026-09-11 用户原话「ux做好看点 给点交互 动效啥的」)**:静态页面 = 不及格。每个 app 至少:① hero 操作有按压反馈与状态过渡(开始/进行中/完成);② 列表增删有 `AnimatedList`/`AnimatedSwitcher` 级别的进出场,不是硬刷新;③ 关键数字(评分/进度/倒计时)用 `TweenAnimationBuilder` 滚动到位,不是瞬间跳变;④ 页面切换用 Material 3 的共享轴或淡入,不用默认硬切。动效要克制(150–300ms、standard easing),是为了「顺手」不是「炫」。
