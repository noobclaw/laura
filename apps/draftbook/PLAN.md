...
## 设计简报 — Draftbook/写长篇(2026-09-25 UI 改版,按 kb/UIUX规矩.md 5.2;取代 §6b 里的「纸与墨」视觉)

1. **一句话**:通勤、排队、睡前,**单手**拿手机,打开后 **3 秒内**回到上次停笔的那一行接着写;写完看一眼今天写了多少。
2. **首屏**:当前这本书的**校样页**——大号衬线书名、章/场景行、上次停笔处的最后几行正文(衬线排印,末尾是插入符),下面是今天的**行规尺**(signature)和一条**书口章节带**。首屏不是书架列表。**主操作「接着写」**在底部拇指栏:屏幕最下方 ~12% 高度内,占底栏右侧约 2/3 宽;左侧两个低频入口「书架」「大纲」(自绘图标,各 48×48)。设置入口是右上角一个自绘小图标(低频,放顶部)。
3. **3 秒首胜(F1)**:冷启动零设置。首次打开 = 一张空白校样页,书名**直接在页上输入**(可留空,占位「未命名」),拇指区按「开始写」**1 次点击**进编辑器、光标就位(新书自带 1 章 1 场景,沿用 `addProject` 的既有语义)。有稿时按「接着写」**1 次点击**回到上次那一场景。不注册、不引导页、不要权限。
4. **原型**:**编辑排版 Editorial**(决策表里空档,无已有 app 占用)。原 §6b 的「纸与墨」属于 Paper 原型(与 daycount 共用),本次作废。与 Paper 原型的 daycount 对比:【字体】打包 Newsreader 衬线(daycount 无打包字体);【形状】0/4/胶囊 + 细线分隔(daycount 12 圆角 + 纸张阴影)。
5. **色彩**:策略 **Restrained**(白底 + 墨色 + 一个"校对蓝铅笔"强调色);色相 **198°**(PIPELINE 第 13 条色相表 190–210° 空档,§6b 已登记;remcard 174° / orbit 223° 均差 ≥20°;ohmbench 的强调是 45° 黄 + 193° 青且为深色蓝图原型,本 app 浅色优先、深色底用暖石墨避开它的海军蓝底)。
   - 浅色:bg `#F7F6F2`(校样纸)· surface `#FFFFFF`(页)· ink `#1B1D1E` · accent `#175F7E`(198°,白底 7.1:1)· signal `#A8321F`(校对红,只用于错误/删改,白底 6.7:1)
   - 深色:bg `#171615`(暖石墨,非纯黑)· surface `#201F1D` · ink `#E8E6E1`(约 90% 白)· accent `#7CBEDB`(198°,底上 8.8:1)· signal `#F08A76`
   - 次要文字:浅 `#5E6367`(页上 6.2:1)/ 深 `#A29E97`(6.8:1);细线:浅 `#DAD6CC` / 深 `#34322F`。强调色只给主按钮、插入符、行规墨线,面积 ≤10%。
6. **字体**:展示字 **Newsreader**(Production Type,**SIL OFL 1.1**;来源 `github.com/google/fonts/ofl/newsreader` 的可变字体 `Newsreader[opsz,wght].ttf` / `Newsreader-Italic[opsz,wght].ttf`,2026-09-25 下载;用 fontTools `varLib.instancer` 固定轴 + `subset` 只留拉丁/数字/标点,保留 kern/liga/tnum/case):`NewsreaderDisplay-Medium.ttf`(opsz 60 / wght 500,51KB,书名与大数字)、`Newsreader-Regular.ttf`(opsz 16 / 400,51KB,稿面正文拉丁部分)、`Newsreader-SemiBold.ttf`(600,51KB)、`Newsreader-Italic.ttf`(400 斜体,56KB,章/场景副行)。许可文本 `assets/fonts/OFL.txt` 并在 `main()` 用 `LicenseRegistry` 注册。**中文展示子集:不用**(书名是用户输入,子集覆盖不到);中文一律回退系统字体(iOS 苹方),`fontFamilyFallback` 写全。UI 控件文字 = 系统字体。数字统一 `tabularFigures`。
7. **形状**:3 个圆角 **0 / 4 / 胶囊**(`DbRadius.none/small/pill`);分隔方式 = **细线描边**(1px hairline,不用卡片阴影);基准间距 **8pt**(半档 4);列表行高 **56pt**(最小 44)。
8. **图形语言**:**排字行规刻度**(印刷厂量行用的 line gauge:细刻度 + 衬线数字)与**书口章节带**(把一本书侧放看到的书口,每章一段,厚度 = 该章字数)。行规显示**今天写的字数 / 每日目标**;书口显示**每章字数占全书比例**。
9. **signature 场景**:行规上的墨线**从上次看到的字数"排"到现在的字数**,笔头带一个校对插入符 ‸,越过目标刻度的那一刻刻度数字变成强调色、触发一次 medium 触感;超过目标后标尺按比例收缩以容纳。由 `store.todayWords` / `store.dailyGoal` 驱动。开启「减少动态效果」后:直接画终态,无笔头、无延伸动画。用 `RepaintBoundary` 隔离。
10. **空状态**:空白校样页本身就是示范——页上是可输入的书名框、「第 1 章 · 场景 1」副行、8 条空排字格线,行规显示 0 / 500;「开始写」按钮在底部拇指栏。版本历史空态 = 一条空的版本时间线(竖细线 + 一个"现在"节点)加一句说明;大纲里空章 = 一条虚线格 + 行内「在这章写一个场景」按钮。都不是"圆底图标 + 标题 + 灰字 + 按钮"。
11. **触感映射**(统一走 `Haptics`,设置页总开关):① 格式按钮 / 每日目标选择 / 状态切换 = `selectionClick`;② 开始写 / 接着写 / 恢复版本 / 导出完成 = `lightImpact`;③ 今天第一次越过每日目标 = `mediumImpact`(每天最多一次)。出错不震。
12. **首屏线框**
```
 ┌──────────────────────────────────┐
 │ Draftbook                    [⚙] │ ← 低频
 │ Night Bus                        │ ← Newsreader Display 34
 │ Chapter 3, The Station · Scene 2 │ ← 斜体
 │ ┌──────────────────────────────┐ │
 │ │…the bus slowed under the     │ │ ← 页:末尾 6 行正文
 │ │ bridge and she counted the   │ │
 │ │ lights until ▍               │ │ ← 插入符
 │ └──────────────────────────────┘ │
 │ 312  of 500 words today          │
 │ |····|····|··‸·|····|····|       │ ← 行规(signature)
 │ ▇▇▇▇▇▇▅▅▅▅▂▂  12,480 words · 3 ch │ ← 书口章节带
 ├──────────────────────────────────┤
 │ [书架] [大纲]  [   接着写  →   ] │ ← 拇指区
 └──────────────────────────────────┘
```
13. **自检**:"换一个类似需求的 app(比如日记),我会不会做成一样的?"——会撞的是"大数字 + 进度条"。所以数字不单独成卡:数字嵌在行规上方、与刻度同一基线系统;进度不用圆角进度条而用印刷行规;首屏主体是**稿子本身的正文**,这是日记/计数类 app 没有的。
