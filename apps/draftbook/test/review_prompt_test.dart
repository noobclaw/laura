...
import 'package:draftbook/core/review_prompt.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:path_provider_platform_interface/path_provider_platform_interface.dart';

/// Points `JsonFileStore` at a throwaway directory so the test never touches
/// the host's real documents folder (and never sees a count left behind by an
/// earlier run).
...
void main() {
  ...
  setUp(() async {
    tmp = await Directory.systemTemp.createTemp('draftbook_review_');
  ...
  test('asks after the third writing session and stays quiet after that',
      () async {
    var asks = 0;
    ...
    expect(asks, 0);
    await ReviewPrompt.noteCoreAction();
    expect(asks, 1, reason: 'third writing session should ask');
    await ReviewPrompt.noteCoreAction();
