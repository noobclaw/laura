...
| `tonaljs/tonal`(`packages/chord-type/data.ts`、`scale-type/data.ts`) | **MIT**(每个 `packages/*/LICENSE` 都在;09-12 G8c 复核纠正了此前「无许可证」的误判)→ 字典仍自写,tonal 的音程串只作为 `test/theory_tonal_test.dart` 的期望值(已署名) | ① 字典格式=「音程串 + 全名 + 别名」;② 和弦扩展音程>八度(9=14 半音)以保留声部意义;③ 音阶/和弦共用同一套音程→音级换算;④ 覆盖面:三和弦/七和弦/扩展/挂留/五声/调式/异域音阶。 | `music/theory.dart` 自写:每条目=半音偏移数组(公式本身就是乐理常识:大三度 4、纯五度 7…),配 zh+en 名与符号;精选 28 种和弦 + 20 种音阶(练习向,不收 alt 爵士堆叠);`degreeLabel`/`scaleDegreeLabel` 自写。 |
