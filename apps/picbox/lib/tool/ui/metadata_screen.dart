    ...
    return JobResult(
      ...
      note: r == null
          ...
          : r.isEmpty
              ...
              : tr(zh: '已移除 ${r.tagCount} 个标签${r.hasGps ? '(含 GPS)' : ''}', en: 'Removed ${r.tagCount} tags${r.hasGps ? ' (incl. GPS)' : ''}'),
    );
  }
      ...
      options: (context, images) {
        if (images.isEmpty) {
          return SectionCard(
            ...
            child: Text(
              tr(
                zh: '照片里通常藏着拍摄地点(GPS)、时间、机型、软件等信息。这里会先列出每张图内嵌了什么,再无损地全部移除——像素一个字节都不动。',
                en: 'Pictures usually carry the place (GPS), time, device and software they came from. This lists what each file contains, then removes all of it losslessly — not a single pixel is touched.',
              ),
              style: text.bodyMedium?.copyWith(height: 1.45),
            ),
          );
        }
        final gpsCount = images.where((s) => _reports[s.id]?.hasGps ?? false).length;
        return Column(
          ...
          children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        tr(zh: '$gpsCount 张图片带有拍摄地点', en: '$gpsCount picture${gpsCount == 1 ? '' : 's'} carry a location'),
                        style: text.bodyMedium?.copyWith(color: cs.onErrorContainer, fontWeight: FontWeight.w600),
                      ),
                    ),
                  ],
                ),
            ...
            for (final s in images) ...[
              _ReportCard(image: s, report: _reports[s.id], loading: _loading.contains(s.id)),
...
class _ReportCard extends StatelessWidget {
  const _ReportCard({required this.image, required this.report, required this.loading});
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(image.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: text.titleSmall),
                      Text(describeImage(image), style: text.bodySmall?.copyWith(color: cs.onSurfaceVariant)),
                    ],
                  ),
                ),
                if (r?.hasGps ?? false)
            ...
            if (loading || r == null)
              Row(
                children: [
                  ...
                  Text(tr(zh: '正在读取…', en: 'Reading…'), style: text.bodySmall),
                ],
              )
            else if (r.isEmpty)
              Row(
                children: [
                  ...
                  Text(tr(zh: '没有发现元数据', en: 'No metadata found'), style: text.bodyMedium),
                ],
              )
            else ...[
              Wrap(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                        tr(zh: '${r.tagCount} 个标签 · ${formatBytes(r.metaBytes)}', en: '${r.tagCount} tags · ${formatBytes(r.metaBytes)}'),
                        ...
                        SizedBox(
                          ...
                          child: Text(_label(e.key),
                              style: text.bodySmall?.copyWith(
                                  color: e.group == 'gps' ? cs.error : cs.onSurfaceVariant,
                                  fontWeight: e.group == 'gps' ? FontWeight.w600 : null)),
                        ),
                        Expanded(
                          child: Text(e.value,
                              maxLines: 2,
