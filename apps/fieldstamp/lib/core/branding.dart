...
abstract final class Branding {
  static const String appNameEn = 'SiteStamp';
  ...
  static String get aboutText => tr(
        zh: 'SiteStamp(现场印记)— 纯离线的 GPS 现场取证相机。'
            '每张照片都会在拍摄瞬间把 GPS 坐标、海拔、方位和时间直接烧入像素。'
            '可导出 PDF 巡检报告或 CSV 台账。无账号、无广告;'
            '你的照片和坐标永远不会离开这台手机。',
        en: 'SiteStamp — an offline GPS field-evidence camera. '
  ...
  static String get privacyPolicy => tr(
        zh: '''
SiteStamp(现场印记)不收集、不存储、不传输任何个人数据。

应用仅使用你设备上的相机和定位传感器,在本机生成带地理水印的照片。它不申请网络权限,不包含任何统计或广告 SDK,也不使用任何第三方服务。你的照片、坐标、项目以及所有导出文件都只保存在你的设备上。

你在应用内创建的照片和数据会在卸载应用时一并删除。当你主动分享或导出照片、PDF 或 CSV 时,文件会通过系统分享面板交给你选择的应用——SiteStamp 本身从不上传任何内容。
''',
        en: '''
SiteStamp does not collect, store, or transmit any personal data.
...
Photos and data you create are removed when you uninstall the app. When you choose to share or export a photo, PDF, or CSV, it is handed to whatever app you pick via the system share sheet — SiteStamp itself never uploads anything.
''',
      );
}
