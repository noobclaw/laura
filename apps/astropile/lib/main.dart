...
import 'tool/astropile_tool.dart';
...
final AstropileTool tool = AstropileTool();

Future<void> main() async {
  WidgetsFlutterBinding.ensureInitialized();
  // Every screen stacks a wide preview over controls; landscape leaves no
  // room for the picture. Portrait only, both platforms (also declared in
  ...
  runApp(const AstroPileApp());
}

class AstroPileApp extends StatelessWidget {
  const AstroPileApp({super.key});
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<String?>(
      ...
      builder: (context, code, _) => MaterialApp(
        ...
        darkTheme: buildAstroTheme(Brightness.dark),
