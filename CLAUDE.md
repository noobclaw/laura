...
- **每 3 天落地 1 个**(定时任务 `app-factory-3d`):**不自己选题,直接取 BACKLOG.md 排名最前的「待做」项**开发,开发全自动,完成后通知用户装机验收。**开发全程必须逐条执行 PIPELINE.md 的「⭐工厂标准作业流程 SOP」(G0-G8:防撞车/取题否决/立项评审/多语言/审计/CI冒烟/用语纪律/变现标准),缺一不可。**
